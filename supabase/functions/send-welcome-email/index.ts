...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!record?.id) return jsonResponse({ error: "record.id requis" }, 400);
    ...
    if (profile.welcome_email_sent) return jsonResponse({ status: "already_sent" });

    const { data: authUser } = await admin.auth.admin.getUserById(record.id);
    const email = authUser?.user?.email;
    if (!email) return jsonResponse({ error: "Email introuvable" }, 404);
